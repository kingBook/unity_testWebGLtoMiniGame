...
namespace WeChatWASM
{
    ...
    public class WXEditorWindow : EditorWindow
    {
        ...
        public static void Open()
        {
            ...
            win.minSize = new Vector2(650, 750);
            win.maxSize = new Vector2(1600, 750);
            win.Show();
            // 打开面板时自动检查更新
        ...
        [MenuItem("微信小游戏 / 包体瘦身--压缩纹理 / ReadMe")]
        ...
        public void LoadData() {
            ...
            totalMemory = config.ProjectConf.MemorySize;
        ...
        private void OnLostFocus() {
            ...
            config.ProjectConf.MemorySize = totalMemory;
        ...
        private void checkNeedCopyDataPackage(bool brotliError)
        {
            ...
            if (brotliError) {
                ...
                throw new Exception("Brotli压缩失败，不可使用代码分包加载资源");
            }
            if ((assetLoadType == 1))
            {
#if UNITY_2020_1_OR_NEWER
        ...
        private string GetPreloadList(string preloadfiles)
        {
            ...
            {
                Debug.LogError($"并非所有预下载的文件都被找到，剩余：{file}");
            }
            return preloadList;
        }
        ...
        public void ModifyWeChatConfigs(bool isFromConvert = false)
        {
            ...
            List<string> files = new List<string> { "game.js", "game.json", "project.config.json", "texture-config.js" };
            if (isFromConvert) {
